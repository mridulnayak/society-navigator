      ...
      <div className="pointer-events-auto bg-slate-900/95 backdrop-blur-md p-4 rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.8)] border border-slate-700">
        ...
        {isLoggedIn && userRole === 'resident' && (
            <form onSubmit={handleUpdateHouseName} className="w-full bg-blue-500/10 border border-blue-500/30 rounded-lg p-3 mb-3 flex flex-col gap-2">
                ...
                <div className="flex gap-2"><input type="text" value={editHouseName} onChange={(e) => setEditHouseName(e.target.value)} className="flex-1 px-3 py-1.5 bg-slate-950 text-white rounded-lg border border-slate-700 focus:outline-none text-xs" /><button type="submit" className="px-3 py-1.5 rounded-lg font-bold text-[10px] text-white bg-blue-600 hover:bg-blue-500">{text.save}</button></div>
            </form>
        )}
